-- Days until first candidate is emailed -- 

SELECT 
    co.id as "Company Id",
    co.name as "Company Name",
    c.id as "Contract Id",
    TIMESTAMPDIFF(DAY,c.start_date,min(ce.created)) AS "Days until first candidate is emailed" -- ce.created - contract start date
FROM contracts c
