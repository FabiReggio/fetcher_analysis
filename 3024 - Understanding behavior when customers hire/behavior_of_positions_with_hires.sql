SELECT 
    temp.hired - temp.undo_hired as "# Positions with hires",
...
FROM (
    SELECT 
        -- CANDIDATE EVENTS METRICS
        (SELECT count (distinct (position_id)) FROM candidate_events WHERE event_id=405) as hired,
        (SELECT count (distinct (position_id)) FROM candidate_events WHERE event_id=412) as undo_hired,
