SELECT temp.c_name as 'company',
       ...
       sum(temp.liked) as Liked,
       sum(temp.disliked) as Disliked,
...
FROM 
  (SELECT c.NAME as 'c_name', 
       ...
       (SELECT Count(*) 
        ...
        -- liked
        (select count(distinct ce.caliber_id) from candidate_events ce where ce.position_id=p.id and ce.event_id in (201) [[and date(ce.created) between {{start_date}} and {{end_date}}]] ) as liked,
        
        -- disliked
        (select count(distinct ce.caliber_id) from candidate_events ce where ce.position_id=p.id and ce.event_id in (202) [[and date(ce.created) between {{start_date}} and {{end_date}}]] ) as disliked
...
WHERE  /*( cc.status <> 4 // We do need to see activity for cancelled people
         AND cc.status <> 3 ) 
       AND */
       pso.is_owner = 1 
