select
    ...
    , temp.contract_id
    , temp.contract_start_date
    , temp.contract_end_date
-- Fetcher leads
    -- Included
    , temp.iflu as included_fetcher_leads_used
    , temp.iflp as included_fetcher_leads_paid
    , div0(temp.iflu, temp.iflp) as included_fetcher_leads_used_rate
    -- Upsells
    , temp.uflu as upsell_fetcher_leads_used
    , temp.uflp as upsell_fetcher_leads_paid
    , div0(temp.uflu, temp.uflp) as upsell_fetcher_leads_used_rate
-- Extension leads
    -- Included
    , temp.ielu as included_extension_leads_used
    , temp.ielp as included_extension_leads_paid
    , div0(temp.ielu, temp.ielp) as included_extension_leads_used_rate
    -- Upsells
    , temp.uelu as upsell_extension_leads_used
    , temp.uelp as upsell_extension_leads_paid
    , div0(temp.uelu, temp.uelp) as upsell_extension_leads_used_rate
from (
    select
        c.company_id
        , c.id as contract_id
