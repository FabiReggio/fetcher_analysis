...
from (
	...
	join positions_batches as pb on pb.batch_id = bc.batch_id
	join batches on batches.id = bc.batch_id
	
	-- NEW JOINS
    join positions p on pb.position_id = p.id 
    join positions_side_owners pso on pso.position_id = pb.position_id and pso.is_owner = 1
    join customer_companies cc on cc.caliber_id = pso.caliber_id
    join companies co on co.id = cc.company_id 
    join calibers as customers on customers.id = cc.caliber_id -- info about the customer
